...
this.image_list = document.querySelectorAll(".carousel-wrapper img");
...
let imageTrasnsition = function (currentIndex, nextIndex) {
  var interval = setInterval(function () {
    ...
    if (dir == 1 && wrapperLeft <= -nextIndex * IMAGE_WIDTH) {
      //reached right image
      wrapper.style.left = -nextIndex * IMAGE_WIDTH + "px";
      setActiveDot(nextIndex);
      clearInterval(interval);
      isTransiting = false;
    } else if (dir == -1 && wrapperLeft >= -nextIndex * IMAGE_WIDTH) {
      //reached left image
      wrapper.style.left = -nextIndex * IMAGE_WIDTH + "px";
      clearInterval(interval);
      setActiveDot(nextIndex);
